...
// Imp Points : 
// 1.remainder cannot be -ve 
// 2.i/p array can also contain -ve number
// 3. (a+b)%k = ((a%k)+(b%k))%k
// 4. The number we get by doing k-currentIndex/x(remainder of first number whose pair to be found) we are on should have the same frequency as the number having remainder x
// 5. we have (a,b) and k , so if a gives remainder x when divided by k . so b will have have remainder as k-x (eg. =  to get (a+b)%k = k , let a  has raminder 1 and k=5 so b will have raminder 4 as  to get (1+4)%5 = 5)
//6. In this key = remainder & value = count of numbers having that same remainder 
